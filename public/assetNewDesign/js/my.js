(function($) {
    ...
    $.fn.loadBroker = function(link, type) {
        $("#loading").show();
        $('#brokerLoad').load(link, function(data, stat, xhr) {
            if (type == "table") {
                $(this).shortTable();
                $('#table-wrapper').hide();
                setTimeout(function() {
                    $("#loading").hide();
                    $('#table-wrapper').fadeIn(500);
                    $('#table-wrapper').show();
                }, 750);
            } else {
                ...
                setTimeout(function() {
                    $("#loading").hide();
                    $('#carouselExampleIndicators').fadeIn(500);
    ...
    $.fn.download = function(e) {
        window.open("manuals/download/" + $(this).attr('link-download'), '_blank');
        return false;
    }

    $.fn.shortTable = function(maxRows = 5) {
        ...
        $(table + '  tr:gt(0)').each(function() {
            trnum++
            $(this).prepend('<td class="numb-list"><h5 class="dark b">' + trnum + '</h5></td>');
            if (trnum > maxRows) {
                $(this).hide()
            }
            if (trnum <= maxRows) {
    ...
    // open new windows
    $.fn.PopupCenter = function(url, title, w, h) {
